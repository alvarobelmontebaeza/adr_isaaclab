...
from isaaclab.utils.math import matrix_from_quat, compute_pose_error, sample_uniform, subtract_frame_transforms, quat_error_magnitude, quat_from_euler_xyz, euler_xyz_from_quat, combine_frame_transforms, quat_unique
...
class AdrIsaaclabEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        # Obtain rotations as 6D representations for better learning continuity: https://arxiv.org/abs/1812.07035
        # Left arm
        left_ee_pos = self.ee_pose_left_b[:, :3]
        left_ee_rot_6d = matrix_from_quat(self.ee_pose_left_b[:, 3:7])[:, :, :2].reshape(self.num_envs, 6)
        
        # Right arm
        right_ee_pos = self.ee_pose_right_b[:, :3]
        right_ee_rot_6d = matrix_from_quat(self.ee_pose_right_b[:, 3:7])[:, :, :2].reshape(self.num_envs, 6)

        # Get target EE poses
        target_left_ee_pos = self._target_ee_pose_left_b[:, :3]
        target_left_ee_rot_6d = matrix_from_quat(self._target_ee_pose_left_b[:, 3:7])[:, :, :2].reshape(self.num_envs, 6)
        target_right_ee_pos = self._target_ee_pose_right_b[:, :3]
        target_right_ee_rot_6d = matrix_from_quat(self._target_ee_pose_right_b[:, 3:7])[:, :, :2].reshape(self.num_envs, 6)
        ...
        obs = torch.cat(
            (
                ...
                right_ee_pos, #3
                right_ee_rot_6d, #6
                target_right_ee_pos, #3
                target_right_ee_rot_6d, #6
                left_ee_pos, #3
                left_ee_rot_6d, #6
                target_left_ee_pos, #3
                target_left_ee_rot_6d, #6
                actions, # 14
            ),
            dim=-1, # Total: 84
        )
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Base rotation reward to encourage the robot to keep its base stable and avoid drifting due to arm motions - we can use the base linear and angular velocities as a proxy for this
        # TODO: Encourage static base by penalizing deviation from default rotation.
        base_rotation = self._robot.data.root_quat_w
        rotation_deviation = quat_error_magnitude(base_rotation, self._default_body_rotation)
        # Base velocity penalty to avoid body drift due to arms coupled motion
        base_linear_vel_penalty = torch.sum(torch.square(self._robot.data.root_lin_vel_b), dim=-1)
        base_angular_vel_penalty = torch.sum(torch.square(self._robot.data.root_ang_vel_b), dim=-1)
        ...
        rewards = {
            ...
            "base_rotation": rotation_deviation * self.cfg.base_rotation_rew_scale * self.step_dt,
